...
<body>
	...
	<form action="./jsysF" method="post">
		<table border="1">
			<tr>
				<td>得意先コード</td>
				<td><input type="text" name="custCode" value="${param.custCode}" /></td>
				<td>得意先名</td>
				<td><c:out value="${requestScope.customer.custName}" /></td>
			</tr>
			<tr>
			<td><button type="submit" name="buttonId" value="V303_01">集計</button>
			<button type="submit" name="buttonId" value="V303_00">クリア</button> 
			</td>
			</tr>
		</table>
	</form>
	...
	<c:if test="${not empty orderList}">
		<table border="1" style="margin: 0 auto" id="data-table">
			...
			<c:forEach var="order" items="${requestScope.orderList}"
				varStatus="status">
				<tr>
					<td nowrap><c:out value="${order.itemCode}" /></td>
					<td nowrap><c:out value="${order.itemName}" /></td>
					<td nowrap><c:out value="${order.totalAmount}" /></td>
					<td nowrap><c:out value="${order.price}" /></td>
